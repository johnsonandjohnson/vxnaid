...
fun gone(): Matcher<View> = ViewMatchers.withEffectiveVisibility(ViewMatchers.Visibility.GONE)
fun visible(): Matcher<View> = ViewMatchers.withEffectiveVisibility(ViewMatchers.Visibility.VISIBLE)
 ...
 */
fun <T : Any> Matcher<View>.clickAtPosition(position: Int, className: KClass<T>) {
    onData(instanceOf(className.java))
        .atPosition(position)?.perform(ViewActions.click()) ?: throw Exception("DataInteraction not found.")
}